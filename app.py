...
def consulta_general():
    try:
        ...
        data=[]
        for row in datos:
            dato={'idbaul':row[0],'Plataforma':row[1],'usuario':row[2],'clave':row[3]}
            data.append(dato)
        cur.close()
        conn.close()
...
def consulta_individual(codigo):
    try:
        conn=conectar('localhost','root','1234','gestor_contrasena')
        cur = conn.cursor()
        cur.execute(""" SELECT * FROM baul where idbaul='{0}' """.format(codigo))
        datos=cur.fetchone()
        cur.close()
        conn.close()
        if datos!=None:
            dato={'idbaul':datos[0],'Plataforma':datos[1],'usuario':datos[2],'clave':datos[3]}
...
def eliminar(codigo):
    try:
        conn=conectar('localhost','root','1234','gestor_contrasena')
        cur = conn.cursor()
        x=cur.execute(""" delete from baul where idbaul={0}""".format(codigo))
        conn.commit()
        cur.close()
        conn.close()
...
def actualizar(codigo):
    try:
        ...
        x=cur.execute(""" update baul set plataforma='{0}',usuario='{1}',clave='{2}' where \
            idbaul={3}""".format(request.json['plataforma'],request.json['usuario'],\
